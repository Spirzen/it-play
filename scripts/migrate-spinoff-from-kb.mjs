/**
 * Копирует React-демо из docs/encyclopedia/9-spinoff (it-knowledge-base) в it-play.
 * Usage: node scripts/migrate-spinoff-from-kb.mjs [--dry-run]
 */
...
const KB_DOCS = path.join(KB_ROOT, 'docs', 'encyclopedia', '9-spinoff');
...
const NAMED_EXPORT_SOURCES = {
  GamePcPlatformPlay: {fileBase: 'GamePlatformPlay', innerExport: 'GamePcPlatformPlay'},
  GamePlayStationPlatformPlay: {
    fileBase: 'GamePlatformPlay',
    innerExport: 'GamePlayStationPlatformPlay',
  },
  GameNintendoPlatformPlay: {
    fileBase: 'GamePlatformPlay',
    innerExport: 'GameNintendoPlatformPlay',
  },
  GameXboxPlatformPlay: {fileBase: 'GamePlatformPlay', innerExport: 'GameXboxPlatformPlay'},
};

const TITLE_MAP = {
  AdobeProductPlay: 'Adobe — продуктовая линейка',
  AttentionManipulationPlay: 'Манипуляции вниманием',
  AutodeskProductPlay: 'Autodesk — продукты',
  BlenderWorkspacePlay: 'Рабочее пространство Blender',
  BlockchainChainPlay: 'Блокчейн — цепочка блоков',
  CommunityCoordinationPlay: 'Координация сообщества',
  CreatorRolesPlay: 'Роли создателей контента',
  CrossContextMemePlay: 'Мем в разных контекстах',
  CryptoTransactionPlay: 'Криптотранзакция',
  CryptoVolatilityPlay: 'Волатильность криптовалют',
  ForumImageboardPlay: 'Форум и имиджборд',
  GameDesignMdaPlay: 'MDA — механики, динамика, эстетика',
  GameDevHub: 'Разработка игр — навигатор',
  GameDevPipelineDemo: 'Пайплайн разработки игр',
  GameDevRoadmapPlay: 'Дорожная карта геймдева',
  GameDevStudioDemo: 'Игровая студия',
  GameDevTeamPlay: 'Команда разработки игр',
  GameFranchisePlay: 'Игровая франшиза',
  GameIndustryEcosystemDemo: 'Экосистема игровой индустрии',
  GameIndustryHub: 'Игровая индустрия — навигатор',
  GameLanguagesPlay: 'Языки в геймдеве',
  GameMobilePlay: 'Мобильная разработка игр',
  GameModelingPlay: '3D-моделирование для игр',
  GameNintendoPlatformPlay: 'Платформа Nintendo',
  GameOptimizationPlay: 'Оптимизация игр',
  GamePcPlatformPlay: 'Платформа PC',
  GamePlayStationPlatformPlay: 'Платформа PlayStation',
  GameQaPlay: 'QA в играх',
  GameStudiesHub: 'Игроведение — навигатор',
  GameTexturesPlay: 'Текстуры в играх',
  GameXboxPlatformPlay: 'Платформа Xbox',
  InternetCultureTimelinePlay: 'Хронология интернет-культуры',
  MemeLifecyclePlay: 'Жизненный цикл мема',
  NetiquettePlay: 'Нетикет',
  ThreeDGraphicsPlay: '3D-графика',
  VerticalSoftwareExplorerPlay: 'Отраслевое ПО — обзор',
};
...
function inferPlayMeta(manifestKey, docPath) {
  ...
  return {
    playSlug: `spinoff/${slug}`,
    category: 'spinoff',
    categoryTitle: 'Энциклопедия · Спин-офф',
    component: slug,
...
function transformSource(source) {
  ...
  out = out.replace(/from '\.\/([^']+\.module\.css)'/g, "from '@/components/demos/$1'");
  out = out.replace(/from "\.\/([^"]+\.module\.css)"/g, 'from "@/components/demos/$1"');
...
function collectSharedDeps(entryFiles) {
    ...
    for (const imp of extractImports(source)) {
      if (imp.startsWith('./shared/') || imp.startsWith('../shared/')) {
        ...
        if (rel.endsWith('.module.css')) {
          const cssPath = path.join(path.dirname(filePath), rel);
          if (fs.existsSync(cssPath) && !cssFiles.has(cssPath)) {
  ...
  return {sharedFiles: [...sharedFiles], demoFiles: [...demoFiles], cssFiles: [...cssFiles]};
}
...
function main() {
  ...
  console.log(`Spinoff components to migrate: ${componentMap.size}`);
  ...
  const {sharedFiles, demoFiles, cssFiles} = collectSharedDeps(entryFiles);
  const allDemoFiles = new Set([...entryFiles, ...demoFiles, ...cssFiles]);
  ...
  console.log(`Manifest entries: ${Object.keys(manifest).length} (+${componentMap.size} spinoff)`);
}
