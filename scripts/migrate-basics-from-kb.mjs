/**
 * Копирует React-демо из docs/encyclopedia/1-basics (it-knowledge-base) в it-play.
 * Usage: node scripts/migrate-basics-from-kb.mjs [--dry-run]
 */
...
const KB_DOCS = path.join(KB_ROOT, 'docs', 'encyclopedia', '1-basics');
...
/** DOM-виджеты энциклопедии — остаются в it-knowledge-base. */
...
/** Именованный экспорт → файл-источник и Inner-комponent для default wrapper. */
const NAMED_EXPORT_SOURCES = {
  GameGenreToysHub: {fileBase: 'GameGenreToysPlay', innerExport: 'GameGenreToysHubInner'},
};

const TITLE_MAP = {
  EndUserSoftwareHub: 'Софт пользователя — хаб',
  SearchEnginesHub: 'Поисковые системы — хаб',
  LanguageIntroHub: 'Обзор языков программирования',
  TechHistoryHub: 'История технологий — хаб',
  TechHistoryPlay: 'История технологий',
  GameGenreToysHub: 'Жанры игр — обзор',
  GameGenreToyPlay: 'Жанр игры — интерактив',
  InstallerWizardPlay: 'Мастер установки',
  IpAddressAnalyzer: 'Анализатор IP-адреса',
  TextEncoderConverter: 'Кодировки текста',
  MatchThreePlay: 'Match-3 — механика игры',
  BIOSemulator: 'Эмулятор BIOS',
  ComputerBootSequence: 'Последовательность загрузки',
  DisplayTechPlay: 'Технологии дисплеев',
  AppConfigStackPlay: 'Стек конфигурации приложения',
  ProgramBehaviorPlay: 'Поведение программы',
  ProgramLifecyclePlay: 'Жизненный цикл программы',
  WindowsProgramTypesPlay: 'Типы программ Windows',
  WebBrowserSimulator: 'Симулятор браузера',
  MessengerSimulator: 'Симулятор мессенджера',
  VideoConferenceSimulator: 'Симулятор видеосвязи',
};
...
function inferPlayMeta(manifestKey, docPath) {
  ...
  return {
    playSlug: `basics/${slug}`,
    category: 'basics',
    categoryTitle: 'Энциклопедия · Основы',
    component: slug,
...
function main() {
  ...
  console.log(`Basics components to migrate: ${componentMap.size}`);
  ...
  console.log(`Manifest entries: ${Object.keys(manifest).length} (+${componentMap.size} basics)`);
}
