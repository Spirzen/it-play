/**
 * Копирует React-демо из docs/encyclopedia/3-data-markup (it-knowledge-base) в it-play.
 * Usage: node scripts/migrate-data-markup-from-kb.mjs [--dry-run]
 */
...
const KB_DOCS = path.join(KB_ROOT, 'docs', 'encyclopedia', '3-data-markup');
...
const TITLE_MAP = {
  BeginnerWebStackHub: 'Веб-стек для новичка',
  BoxModelPlay: 'CSS Box Model',
  DataStructureTable: 'Табличная структура данных',
  DataStructureHierarchy: 'Иерархическая структура данных',
  DataStructureGraph: 'Граф как структура данных',
  DataStructureLinear: 'Линейные структуры данных',
  DataStructureKeyValue: 'Key-Value структуры',
  DataStructureStack: 'Стек — структура данных',
  CassandraCqlPlay: 'Cassandra CQL',
  MongoShellPlay: 'MongoDB Shell',
  SqlInsertTrainer: 'SQL INSERT — тренажёр',
  SqlUpdateTrainer: 'SQL UPDATE — тренажёр',
  SqlDeleteTrainer: 'SQL DELETE — тренажёр',
  LanguageIntroPlay: 'Обзор языка разметки/данных',
  BannerCarouselPlay: 'CSS-карусель баннеров',
  CssHtmlBridgePlay: 'Мост HTML ↔ CSS',
  ERDDemo: 'ERD — диаграмма сущностей',
  AiDataAnalysisPlay: 'AI-анализ данных',
  BigDataFiveVsPlay: 'Big Data — 5V',
  GisDemo: 'GIS — геоданные',
  PythonDataStatsPlay: 'Python — описательная статистика',
  DataGovernancePlay: 'Data Governance',
  SqlExplainPlanPlay: 'EXPLAIN — план запроса',
  JsonPlayDemo: 'JSON — интерактив',
  MemcachedCachePlay: 'Memcached — кэш',
  InformationEntropyDemo: 'Энтропия информации',
  MarkdownLivePlay: 'Markdown — live preview',
  EtlEltOrchestrationPlay: 'ETL/ELT — оркестрация',
  SqlProcedureTriggerPlay: 'SQL процедуры и триггеры',
  ClusteringPlay: 'Кластеризация',
  MemcachedShellPlay: 'Memcached Shell',
  GraphTraversalPlay: 'Обход графа (Cypher)',
  DataScienceRolesPlay: 'Роли в Data Science',
  RedisCacheDemo: 'Redis — кэш и типы данных',
  XsdValidationDemo: 'XSD — валидация XML',
  XsltTransformDemo: 'XSLT — трансформация',
  XPathPlayDemo: 'XPath — запросы к XML',
  DbIndexVisualizerPlay: 'Визуализация индексов БД',
  VectorSimilarityPlay: 'Векторное сходство',
  MemoryAddressingDemo: 'Адресация памяти',
  LogicTruthTableDemo: 'Таблицы истинности',
  SimplexStepTrainer: 'Симплекс — пошаговый тренажёр',
  LinearProgrammingPlayground: 'Линейное программирование',
  ResponsiveViewportPlay: 'Адаптивный viewport',
  DiscreteMathDemo: 'Дискретная математика',
  MarshalingDemo: 'Маршалинг данных',
  XamlLayoutPlay: 'XAML — разметка',
  FiniteAutomatonDemo: 'Конечный автомат',
  ComplexityDemo: 'Сложность алгоритмов',
  CausalAnalysisPlay: 'Причинно-следственный анализ',
  SqlViewPlay: 'SQL VIEW',
  StyleInjectionPlay: 'Инъекция стилей',
  SqlCtePlay: 'SQL CTE',
};
...
function main() {
  ...
  console.log(`Data-markup components to migrate: ${componentMap.size}`);
  ...
  console.log(`Manifest entries: ${Object.keys(manifest).length} (+${componentMap.size} data-markup)`);
}
