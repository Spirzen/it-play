/**
 * Копирует React-демо из docs/encyclopedia/8-infra-security (it-knowledge-base) в it-play.
 * Usage: node scripts/migrate-infra-security-from-kb.mjs [--dry-run]
 */
...
const KB_DOCS = path.join(KB_ROOT, 'docs', 'encyclopedia', '8-infra-security');
...
const TITLE_MAP = {
  AnsibleRunPlay: 'Ansible — запуск playbook',
  ApiDesignPlay: 'Проектирование API',
  AuditLogCorrelationPlay: 'Корреляция audit-логов',
  AzureReposPlay: 'Azure Repos',
  CiaTriadPlay: 'Триада CIA',
  CicdAuthFlowPlay: 'Аутентификация в CI/CD',
  CodeProtectionPlay: 'Защита кода',
  CodeSafetyPlay: 'Безопасность кода',
  DebPackageExplorerPlay: 'Структура .deb-пакета',
  DeployStrategiesPlay: 'Стратегии деплоя',
  DevOpsCheatSheetPlay: 'DevOps — шпаргалка',
  DevOpsRoleComparePlay: 'Роли DevOps',
  DevOpsToolsStackPlay: 'Стек инструментов DevOps',
  DockerHardeningPlay: 'Hardening Docker',
  GitCommandsPlay: 'Команды Git',
  GitFlagsPlay: 'Флаги Git',
  GitFlowPlay: 'Git Flow',
  GitHubActionsWorkflowPlay: 'GitHub Actions workflow',
  GitInternalsPlay: 'Внутреннее устройство Git',
  GitLabCiPipelinePlay: 'GitLab CI pipeline',
  GitSvnComparePlay: 'Git vs SVN',
  GoMicroservicePlay: 'Go-микросервис',
  IacLifecyclePlay: 'Жизненный цикл IaC',
  LowCodeRolloutPlay: 'Rollout low-code',
  LowNoCodeDemo: 'Low-code / No-code',
  OrderDeskIntegrationPlay: 'Order Desk — REST и WebSocket',
  OwaspTop10ExplorerPlay: 'OWASP Top 10',
  PipelineLifecyclePlay: 'Жизненный цикл пайплайна',
  PulumiIacPlay: 'Pulumi IaC',
  ServiceMeshTrafficPlay: 'Service Mesh — трафик',
  SreMetricsPlay: 'SRE-метрики',
  TerraformPlanPlay: 'Terraform plan',
  ThreatHuntChecklistPlay: 'Чеклист threat hunting',
  WebhooksDeliveryPlay: 'Доставка webhooks',
};
...
function main() {
  ...
  console.log(`Infra-security components to migrate: ${componentMap.size}`);
  ...
  console.log(`Manifest entries: ${Object.keys(manifest).length} (+${componentMap.size} infra-security)`);
}
