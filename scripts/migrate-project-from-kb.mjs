/**
 * Копирует React-демо из docs/encyclopedia/7-project (it-knowledge-base) в it-play.
 * Usage: node scripts/migrate-project-from-kb.mjs [--dry-run]
 */
...
const KB_DOCS = path.join(KB_ROOT, 'docs', 'encyclopedia', '7-project');
...
const TITLE_MAP = {
  AnalyticsSectionHub: 'Аналитика — хаб раздела',
  ArchitectureDiagramStudio: 'Студия архитектурных диаграмм',
  BusinessAnalystRoleDemo: 'Роль бизнес-аналитика',
  CicdDemo: 'CI/CD пайплайн',
  ConfluenceWikiDemo: 'Confluence / Wiki',
  CyclomaticComplexityPlay: 'Цикломатическая сложность',
  DiagramStudio: 'Студия диаграмм',
  DocumentationArchitecturePlay: 'Архитектура документации',
  DomainModelPlay: 'Доменная модель',
  FigmaPrototypeStudio: 'Прототип в Figma',
  GangOfFourCatalog: 'Каталог GoF',
  IntegrationApiTestPlay: 'API-интеграционные тесты',
  IntegrationFlowPlay: 'Поток интеграционного тестирования',
  JiraTaskTrackerDemo: 'Jira — трекер задач',
  LoadTestSimulatorPlay: 'Симулятор нагрузочного тестирования',
  NamingConventionPlay: 'Соглашения об именовании',
  OopClassArchetypesDemo: 'ООП — архетипы классов',
  PseudoDocumentEditor: 'Редактор псевдодокумента',
  RequirementsAnalysisDemo: 'Анализ требований',
  ScalingDemo: 'Масштабирование',
  SoftwareLifecycleDemo: 'Жизненный цикл ПО',
  SystemAnalystRoleDemo: 'Роль системного аналитика',
  SystemDiscoveryDemo: 'Исследование системы',
  TestAutomationRoiPlay: 'ROI автоматизации тестов',
  TestingLifecyclePlay: 'Жизненный цикл тестирования',
  TestingSectionHub: 'Тестирование — хаб раздела',
  Timer: 'Таймер',
  UiRegistrationTestPlay: 'UI-тест регистрации',
  UnitTestPytestPlay: 'Unit-тесты (pytest)',
};
...
function inferPlayMeta(manifestKey, docPath) {
  ...
  return {
    playSlug: `project/${slug}`,
    category: 'project',
    categoryTitle: 'Энциклопедия · Проект',
    component: slug,
...
function main() {
  ...
  console.log(`Project components to migrate: ${componentMap.size}`);
  ...
  console.log(`Manifest entries: ${Object.keys(manifest).length} (+${componentMap.size} project)`);
}
