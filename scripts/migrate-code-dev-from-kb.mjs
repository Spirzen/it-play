/**
 * Копирует React-демо из docs/encyclopedia/4-code-dev (it-knowledge-base) в it-play.
 * Usage: node scripts/migrate-code-dev-from-kb.mjs [--dry-run]
 */
...
const KB_DOCS = path.join(KB_ROOT, 'docs', 'encyclopedia', '4-code-dev');
...
const TITLE_MAP = {
  SearchVisualizer: 'Визуализация поиска',
  SortingVisualizer: 'Визуализация сортировки',
  CleanArchitectureLayersPlay: 'Слои чистой архитектуры',
  BuildProcessFlow: 'Процесс сборки',
  ModuleDependencyGraph: 'Граф зависимостей модулей',
  CodeTransformationDemo: 'Трансформация кода',
  ORMDemo: 'ORM — объектно-реляционное отображение',
  DesktopWindowPlay: 'Окно десктоп-приложения',
  ProcessThreadVisualizer: 'Процессы и потоки',
  OopAbstractionDemo: 'ООП — абстракция',
  ExceptionHandlingDemo: 'Обработка исключений',
  ProgrammingTasksPlay: 'Задачи по программированию',
  ExecutionPerformancePlay: 'Производительность выполнения',
  VariableLifecyclePlay: 'Жизненный цикл переменной',
  OperatorsPlay: 'Операторы',
  LoopsSimulator: 'Симулятор циклов',
  ConditionalFlowPlay: 'Условный поток выполнения',
  ProcessorExecutionPlay: 'Выполнение на процессоре',
  ThreadLifecycleDemo: 'Жизненный цикл потока',
  ProcessLifecycleDemo: 'Жизненный цикл процесса',
  PromiseAsyncDemo: 'Promise и async/await',
  BytecodeVmPlay: 'Виртуальная машина байт-кода',
  FunctionSimulator: 'Симулятор функций',
  MethodCallSimulator: 'Симулятор вызова методов',
  GarbageCollectorDemo: 'Сборщик мусора',
  ObjectLifecycleSimulator: 'Жизненный цикл объекта',
  GitBranchMergePlay: 'Git — ветки и merge',
  ProjectStructureExplorer: 'Структура проекта',
  RefactoringDemo: 'Рефакторинг',
  QrCodeGenerator: 'Генератор QR-кода',
  MetaprogrammingPhasesPlay: 'Фазы метапрограммирования',
  PetProjectPlannerPlay: 'Планировщик pet-проекта',
  DevPrinciplesPlay: 'Принципы разработки',
  ProgrammingParadigmPlay: 'Парадигмы программирования',
  DeadCodeScannerPlay: 'Сканер мёртвого кода',
  FrameworkDemo: 'Фреймворки',
  SolidDemo: 'SOLID',
  ParallelSpeedupLab: 'Ускорение параллельных вычислений',
  RooflinePlay: 'Roofline-модель',
  OopBasicsDemo: 'ООП — основы',
  LanguageAbstractionPlay: 'Уровни абстракции языка',
  WebViewBridgePlay: 'WebView-мост',
  LibraryConnectionDemo: 'Подключение библиотек',
  DeveloperPortfolioPlay: 'Портфолио разработчика',
  OopPolymorphismDemo: 'ООП — полиморфизм',
  OopInheritanceDemo: 'ООП — наследование',
  OopEncapsulationDemo: 'ООП — инкапсуляция',
  CpuRegistersPlay: 'Регистры CPU',
  CallChainPlay: 'Цепочка вызовов',
  RaceConditionDemo: 'Состояние гонки',
  BitRepresentationPlay: 'Битовое представление',
  GraphRepresentationPlay: 'Представление графа',
  PackagePublishPlay: 'Публикация пакета',
  DIDemo: 'Dependency Injection',
  DIPDemo: 'Dependency Inversion Principle',
  HexNotationPlay: 'Шестнадцатеричная запись',
  UndefinedBehaviorDemo: 'Неопределённое поведение',
};
...
function main() {
  ...
  console.log(`Code-dev components to migrate: ${componentMap.size}`);
  ...
  console.log(`Manifest entries: ${Object.keys(manifest).length} (+${componentMap.size} code-dev)`);
}
