...
import {
  ...
  runCrud,
  sqlForOp,
} from '@/components/shared/kb/cppDatabaseEngine';
...
const MODES = [
  ...
  {id: 'lifecycle', label: 'RAII и жизненный цикл'},
];

function PersonsTable({persons, highlightIds = new Set()}) {
  return (
    <div className={styles.tableWrap}>
      <table className="it-demo__table">
        ...
        <tbody>
          {persons.length === 0 ? (
            <tr>
              <td colSpan={PERSON_COLUMNS.length} style={{textAlign: 'center'}}>
                Таблица пуста
              </td>
            </tr>
          ) : (
...
function StackMode() {
  ...
  return (
    <>
      ...
      <div className={styles.stackDiagram} aria-label="Стек C++ — клиентский API — СУБД">
...
function CrudMode() {
  const [persons, setPersons] = useState(() => clonePersons(INITIAL_PERSONS));
  const [layerId, setLayerId] = useState('sqlite3');
  ...
  const cppCode = cppCodeForLayer(layerId, opId, params);

  const execute = () => {
    const result = runCrud(persons, opId, params);
  ...
  const resetDb = () => {
    setPersons(clonePersons(INITIAL_PERSONS));
    setMessage(null);
  ...
  return (
    <>
      ...
      <div className={styles.crudForm}>
        {(opId === 'read' || opId === 'update' || opId === 'delete') && (
          <div className={styles.crudField}>
            <label htmlFor="cpp-db-id">id (пусто = age ≥ 25)</label>
            <input
              id="cpp-db-id"
              type="number"
              min="1"
        ...
        {(opId === 'create' || opId === 'update') && (
          <>
            <div className={styles.crudField}>
              <label htmlFor="cpp-db-name">name</label>
              <input
                id="cpp-db-name"
                value={form.name}
            ...
            <div className={styles.crudField}>
              <label htmlFor="cpp-db-age">age</label>
              <input
                id="cpp-db-age"
                type="number"
                min="0"
      ...
      <PersonsTable persons={persons} highlightIds={highlightIds} />
      ...
      <div className={styles.layout2}>
        ...
        <div>
          ...
          <pre className={styles.codePanel}>{cppCode}</pre>
        </div>
      </div>
    </>
  );
...
function LayersMode() {
  const [opId, setOpId] = useState('read');
  const params = {id: 2, name: 'Bob', age: 36};
  ...
  return (
    <>
      ...
      <p className={styles.stepDetail} style={{textAlign: 'center', marginBottom: '0.65rem'}}>
        Одна операция — четыре уровня абстрации. Внизу всё равно SQL и нативный API.
      </p>
      ...
      <div className={styles.layerCompare}>
        {ACCESS_LAYERS.map((layer) => (
          <div key={layer.id} className={styles.layerCard}>
            ...
            <pre>{cppCodeForLayer(layer.id, opId, params)}</pre>
          </div>
        ))}
      </div>
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="SQLite3, RAII, CRUD и сравнение sqlite3 · SOCI · sqlpp11 · ODB"
      >
        ...
        <p className={styles.footer}>
          {mode === 'stack' &&
            'C++ не имеет встроенной СУБД — доступ через нативные библиотеки (SQLite3, libpq, ODBC). RAII гарантирует освобождение соединений.'}
          {mode === 'crud' &&
            'Параметризованные запросы обязательны: sqlite3_bind_* или soci::use — никогда не склеивайте SQL из строк пользователя.'}
          {mode === 'layers' &&
            'ODB и sqlpp11 добавляют типобезопасность, но требуют генерации кода или шаблонного DSL. SOCI — золотая середина.'}
          {mode === 'lifecycle' &&
            'Оборачивайте sqlite3* в unique_ptr с custom deleter или классы Connection/Statement с деструкторами — как в статье про RAII.'}
        </p>
      </DemoCard>
    </DemoShell>
  );
...
export default CppDatabasePlayInner;
