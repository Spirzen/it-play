...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/juliaEcosystemEngine';
import styles from '@/components/demos/JuliaEcosystemPlay.module.css';
...
function JuliaEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('diffeq');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('repl');
  ...
  const toggleGpu = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('gpu');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема Julia и HPC-архитектура"
        subtitle="JIT и LLVM, Pkg, граф модулей, проекты DiffEq / Flux / Genie и цепочка от исходника до native-кода"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('gpu')}
                  onChange={(e) => toggleGpu(e.target.checked)}
                />
                GPU / ML слой (CUDA.jl · Flux · Zygote)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>GPU</code> — исчезнут рёбра к CUDA, Flux и
              Zygote; останется CPU-пайплайн с DifferentialEquations.
            </p>
...
export default JuliaEcosystemPlayInner;
