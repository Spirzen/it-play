...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/phpEcosystemEngine';
import styles from '@/components/demos/PhpEcosystemPlay.module.css';
...
function PhpEcosystemPlayInner() {
  ...
  const [pickedNode, setPickedNode] = useState('index');
  const [archId, setArchId] = useState('laravel');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('psr4');
  ...
  const toggleQueue = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('reports');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема PHP"
        subtitle="Слои стека, Composer, граф зависимостей, структура Laravel/Symfony/WordPress и жизненный цикл запроса"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                ...
                />
                Фоновые задачи (SendReportJob + Horizon)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в графе. Отключите очередь — исчезнут SendReportJob, Horizon и связи с
              Redis.
            </p>
            <div className={styles.bundleBox}>
              {bundle.autoload.map((c) => (
                <span key={c} className={styles.bundleChip}>
                  {c}
                </span>
              ))}
              {bundle.processes.map((p) => (
                <span key={p} className={styles.bundleChip}>
                  {p}
                </span>
              ))}
              <span className={styles.bundleChip}>Composer-пакетов: {bundle.composerCount}</span>
            </div>
          </>
        )}
...
export default PhpEcosystemPlayInner;
