...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/scalaEcosystemEngine';
import styles from '@/components/demos/ScalaEcosystemPlay.module.css';
...
function ScalaEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('play-service');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('sbt-single');
  ...
  const toggleActors = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) next.add('actors');
      else next.delete('actors');
      return next;
    });
  }, []);
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="Слои экосистемы, граф библиотек, структура Play/ZIO/Pekko, сборка sbt и мультимодули"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('actors')}
                  onChange={(e) => toggleActors(e.target.checked)}
                />
                Акторная подсистема (actors/)
              </label>
            </div>
            ...
            <div className={styles.graphWrap}>
              <svg className={styles.graphSvg} viewBox="0 0 400 260" preserveAspectRatio="xMidYMid meet">
                {edges.map(([from, to]) => {
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в classpath. Отключите <code>actors/</code> — исчезнут связи с pekko и
              часть ZIO-стримов.
            </p>
            <div className={styles.bundleBox}>
              ...
              <span className={styles.bundleChip}>Maven в графе: {bundle.mavenCount}</span>
            </div>
          </>
        )}
...
export default ScalaEcosystemPlayInner;
