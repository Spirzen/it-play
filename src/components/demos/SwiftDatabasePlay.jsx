...
import {
  ...
  runCrud,
  sqlForOp,
} from '@/components/shared/kb/swiftDatabaseEngine';
...
function StackMode() {
  ...
  return (
    <>
      ...
      <div className={styles.stackDiagram} aria-label="Стек Swift — SQLite — хранилище">
...
function CrudMode() {
  ...
  const [layerId, setLayerId] = useState('grdb');
  ...
  const swiftCode = swiftCodeForLayer(layerId, opId, params);
  ...
  return (
    <>
      ...
      <div className={styles.crudForm}>
        {(opId === 'read' || opId === 'update' || opId === 'delete') && (
          <div className={styles.crudField}>
            <label htmlFor="swift-db-id">id книги (пусто = все available)</label>
            <input
              id="swift-db-id"
              type="number"
              min="1"
        ...
        {(opId === 'create' || opId === 'update') && (
          <>
            <div className={styles.crudField}>
              <label htmlFor="swift-db-title">title</label>
              <input
                id="swift-db-title"
                value={form.title}
            ...
            <div className={styles.crudField}>
              <label htmlFor="swift-db-author">author</label>
              <input
                id="swift-db-author"
                value={form.author}
            ...
            {opId === 'create' && (
              <>
                <div className={styles.crudField}>
                  <label htmlFor="swift-db-isbn">isbn</label>
                  <input
                    id="swift-db-isbn"
                    value={form.isbn}
                ...
                <div className={styles.crudField}>
                  <label htmlFor="swift-db-year">published_year</label>
                  <input
                    id="swift-db-year"
            ...
            {opId === 'update' && (
              <div className={styles.crudField}>
                <label htmlFor="swift-db-avail">available</label>
                <select
                  id="swift-db-avail"
      ...
      <div className={styles.layout2}>
        <div>
          <span className={styles.panelLabel}>SQL (что уходит в SQLite / PostgreSQL)</span>
          <pre className={styles.sqlPanel}>{sql}</pre>
        </div>
        <div>
          <span className={styles.panelLabel}>
            Swift — {ACCESS_LAYERS.find((l) => l.id === layerId)?.label}
          </span>
          <pre className={styles.codePanel}>{swiftCode}</pre>
        </div>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <p className={styles.stepDetail} style={{textAlign: 'center', marginBottom: '0.65rem'}}>
        Одна операция — четыре уровня абстрации. На iOS чаще SQLite; на сервере — Fluent и PostgreSQL.
      </p>
      ...
      <div className={styles.layerCompare}>
        {ACCESS_LAYERS.map((layer) => (
          <div key={layer.id} className={styles.layerCard}>
            ...
            <pre>{swiftCodeForLayer(layer.id, opId, params)}</pre>
          </div>
        ))}
      </div>

      <p className={styles.panelLabel} style={{marginTop: '0.75rem'}}>
        Итоговый SQL (пример для GRDB / SQLite.swift)
      </p>
...
function SwiftDatabasePlayInner() {
  const [mode, setMode] = useState('stack');

  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Swift и база данных"
        subtitle="SQLite, GRDB, Core Data, Fluent — подключение, CRUD и поток данных"
      >
        ...
        <p className={styles.footer}>
          {mode === 'stack' &&
            'На iOS приложение не пишет в .sqlite3 напрямую: между кодом и файлом — GRDB, SQLite.swift или Core Data. На сервере Vapor использует Fluent и PostgreSQL.'}
          {mode === 'crud' &&
            'Параметризованные запросы (? и именованные аргументы) защищают от SQL-инъекций. DatabaseQueue.write сериализует запись; read допускает параллельность при WAL.'}
          {mode === 'layers' &&
            'Core Data — объектная модель, не SQL-first. Fluent — полноценный ORM для серверного Swift. Выбор зависит от платформы и сложности домена.'}
          {mode === 'lifecycle' &&
            'Тяжёлые операции с БД — в фоне (Task.detached, backgroundContext). UI обновляйте на MainActor после await или через ValueObservation.'}
        </p>
      </DemoCard>
    </DemoShell>
  );
...
export default SwiftDatabasePlayInner;
