...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/pythonEcosystemEngine';
import styles from '@/components/demos/PythonEcosystemPlay.module.css';
...
function PythonEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('django');
  ...
  const toggleTasks = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('tasks');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="Слои стека, граф импортов, структура Django/FastAPI/Flask, venv, pip и упаковка"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('tasks')}
                  onChange={(e) => toggleTasks(e.target.checked)}
                />
                Фоновые задачи Celery (app/tasks)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в графе. Отключите <code>app/tasks</code> — исчезнут рёбра к celery и
              redis.
            </p>
...
export default PythonEcosystemPlayInner;
