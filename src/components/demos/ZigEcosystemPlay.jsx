...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/zigEcosystemEngine';
import styles from '@/components/demos/ZigEcosystemPlay.module.css';
...
function ZigEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('http-server');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('multi');
  ...
  const toggleComptime = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('comptime_job');
        next.add('cssl');
      } else {
        next.delete('comptime_job');
      }
      return next;
    });
  }, []);
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Архитектура и экосистема Zig"
        subtitle="Слои стека, граф зависимостей, структура проекта, zig build и подключение пакетов"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('comptime_job')}
                  onChange={(e) => toggleComptime(e.target.checked)}
                />
                comptime-шаг в build.zig (таблицы + @cImport ssl)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>build/gen.zig</code> — исчезнут comptime-связи
              с domain и опциональный @cImport.
            </p>
...
export default ZigEcosystemPlayInner;
