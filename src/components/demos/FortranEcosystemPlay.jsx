...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/fortranEcosystemEngine';
import styles from '@/components/demos/FortranEcosystemPlay.module.css';
...
function FortranEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('cmake');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('f90mod');
  ...
  const toggleIoChain = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      IO_CHAIN_IDS.forEach((id) => {
        if (on) next.add(id);
        else next.delete(id);
      });
      return next;
  ...
  const toggleMpi = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) next.add('mpi');
      else next.delete('mpi');
      return next;
    });
  }, []);
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Архитектура и экосистема Fortran"
        subtitle="Слои HPC-стека, граф USE и библиотек, CMake/fpm-проекты, цепочка .f90 → .mod → линковка"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('io')}
                  onChange={(e) => toggleIoChain(e.target.checked)}
                />
                Модуль I/O и NetCDF (mod_io)
              </label>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('mpi')}
                  onChange={(e) => toggleMpi(e.target.checked)}
                />
                Распределённый запуск (MPI)
              </label>
            </div>
            ...
            <div className={styles.graphWrap}>
              <svg className={styles.graphSvg} viewBox="0 0 400 220" preserveAspectRatio="xMidYMid meet">
                {edges.map(([from, to]) => {
                  ...
                  if (!a || !b) return null;
                  const optional = a.type === 'optional' || b.type === 'optional';
                  return (
                    <line
                      key={`${from}-${to}`}
                      x1={a.x}
                      ...
                      x2={b.x}
                      y2={b.y}
                      className={clsx(styles.graphEdge, optional && styles.graphEdgeLazy)}
                    />
                  );
                })}
              </svg>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Снимите I/O — исчезнут <code>mod_io</code> и{' '}
              <code>libnetcdf</code>; без MPI пропадут рёбра к <code>libmpi</code>.
            </p>
...
export default FortranEcosystemPlayInner;
