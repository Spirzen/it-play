...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/assemblerEcosystemEngine';
import styles from '@/components/demos/AssemblerEcosystemPlay.module.css';
...
function TreeDirRow({depth, open, name, onToggle}) {
  return (
    <div
...
function AssemblerEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('linux-syscall');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [linkId, setLinkId] = useState('bare');
  ...
  const linkModel = LINK_MODELS.find((m) => m.id === linkId) ?? LINK_MODELS[0];
  ...
  const toggleSimd = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('simd');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="Слои от CPU до ОС, граф модулей и библиотек, структура проекта, цепочка NASM → ld → ELF"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('simd')}
                  onChange={(e) => toggleSimd(e.target.checked)}
                />
                Опциональный SIMD-модуль + libcrypto.a
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в линковке. Отключите <code>simd.asm</code> — исчезнут связи с{' '}
              <code>libcrypto.a</code>.
            </p>
        ...
        {tab === 'structure' && (
          <>
            <div className="it-demo__tabs" style={{marginBottom: '0.5rem', flexWrap: 'wrap'}}>
              {ARCH_PRESETS.map((p) => (
                <button
                  key={p.id}
                  type="button"
        ...
        {tab === 'modules' && (
          <>
            <div className={styles.modGrid}>
              {LINK_MODELS.map((m) => (
                <button
                  key={m.id}
                  type="button"
                  className={clsx(styles.modCard, linkId === m.id && styles.modCardActive)}
            ...
            <pre className={styles.codeBlock}>{linkModel.syntax}</pre>
            <p className={styles.hint}>
              <strong>Особенности:</strong> {linkModel.traits.join(' · ')}
            </p>
            <p className={styles.hint}>
              <strong>Инструменты:</strong> {linkModel.tools}
            </p>
            <p className={styles.hint}>
              <strong>Где встречается:</strong> {linkModel.use}
            </p>
          </>
        )}
      </DemoCard>
...
export default AssemblerEcosystemPlayInner;
