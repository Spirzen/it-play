...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/luaEcosystemEngine';
import styles from '@/components/demos/LuaEcosystemPlay.module.css';
...
function LuaEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('love');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('require');
  ...
  const toggleNetwork = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('net');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема Lua и Luau"
        subtitle="Слои стека, граф require и LuaRocks, структура LÖVE / Roblox / OpenResty, сборка и модули"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('net')}
                  onChange={(e) => toggleNetwork(e.target.checked)}
                />
                Сетевой модуль lib/net (luasocket, dkjson)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в графе. Снимите галочку сети — исчезнут рёбра к luasocket и dkjson.
            </p>
...
export default LuaEcosystemPlayInner;
