...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/rustEcosystemEngine';
import styles from '@/components/demos/RustEcosystemPlay.module.css';
...
function RustEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('axum-api');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [crateId, setCrateId] = useState('lib-bin');
  ...
  const crateModel = CRATE_MODELS.find((m) => m.id === crateId) ?? CRATE_MODELS[0];
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="Слои стека, граф crate-зависимостей, структура проекта, сборка Cargo и организация модулей"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                ...
                />
                Фоновый worker (tokio::spawn + sqlx)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>worker/</code> — исчезнут рёбра к sqlx из
              фоновых задач.
            </p>
        ...
        {tab === 'crates' && (
          <>
            <div className={styles.modGrid}>
              {CRATE_MODELS.map((m) => (
                <button
                  key={m.id}
                  type="button"
                  className={clsx(styles.modCard, crateId === m.id && styles.modCardActive)}
                  style={{'--mod-color': m.color}}
                  onClick={() => setCrateId(m.id)}
                >
            ...
            <pre className={styles.codeBlock}>{crateModel.syntax}</pre>
            <p className={styles.hint}>
              <strong>Особенности:</strong> {crateModel.traits.join(' · ')}
            </p>
            <p className={styles.hint}>
              <strong>Инструменты:</strong> {crateModel.tools}
            </p>
            <p className={styles.hint}>
              <strong>Где встречается:</strong> {crateModel.use}
            </p>
          </>
        )}
      </DemoCard>
...
export default RustEcosystemPlayInner;
