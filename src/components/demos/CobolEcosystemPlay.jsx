...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/cobolEcosystemEngine';
import styles from '@/components/demos/CobolEcosystemPlay.module.css';
...
function CobolEcosystemPlayInner() {
  ...
  const [pickedNode, setPickedNode] = useState('payroll');
  const [archId, setArchId] = useState('batch-zos');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('copy-call');
  ...
  const toggleOnline = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('cics');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Архитектура и экосистема COBOL"
        subtitle="z/OS и batch, CICS, COPYbook, сборка load module и связь с современными API"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('cics')}
                  onChange={(e) => toggleOnline(e.target.checked)}
                />
                Онлайн: CICS + REST (z/OS Connect)
              </label>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('db2')}
                  onChange={(e) => {
                    setEnabledNodes((prev) => {
                      const next = new Set(prev);
                      if (e.target.checked) next.add('db2');
                      else next.delete('db2');
                      return next;
                    });
                  }}
                />
                DB2 вместо VSAM для чтения клиента
              </label>
            </div>
            ...
            <div className={styles.graphWrap}>
              ...
              <div className={styles.graphNodes}>
                {DEP_NODES.map((n) => {
                  ...
                  const locked = n.id === 'payroll' || n.id === 'jcl';
                  return (
                    <button
                      key={n.id}
                      type="button"
                      ...
                      onClick={() => {
                        setPickedNode(n.id);
                        if (!locked) toggleNode(n.id);
                      }}
                      title={locked ? 'Ядро приложения' : on ? 'Клик — исключить' : 'Клик — включить'}
                    >
                      {n.label}
                    </button>
                  );
            ...
            <p className={styles.hint}>
              ...
              Сценарий payroll: JCL запускает load module, PAYROLL читает VSAM через COPY CUSTREC, вызывает ACCTLOG.
              Включите CICS — появятся узлы онлайн-транзакции и API-шлюза.
            </p>
...
export default CobolEcosystemPlayInner;
