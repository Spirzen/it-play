...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/cppEcosystemEngine';
import styles from '@/components/demos/CppEcosystemPlay.module.css';
...
function CppEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('qt-desktop');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [pkgId, setPkgId] = useState('cmake');
  ...
  const pkg = PACKAGE_MODELS.find((p) => p.id === pkgId) ?? PACKAGE_MODELS[0];
  ...
  const toggleWorker = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('worker');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="Слои стека, граф библиотек, структура проекта, CMake-сборка и модели зависимостей"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('worker')}
                  onChange={(e) => toggleWorker(e.target.checked)}
                />
                Фоновый поток (Boost.Asio)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в линковке. Отключите <code>src/worker/</code> — исчезнут рёбра к Asio и
              часть связей с core.
            </p>
        ...
        {tab === 'packages' && (
          <>
            <PkgGrid pkgId={pkgId} setPkgId={setPkgId} />
            <pre className={styles.codeBlock}>{pkg.syntax}</pre>
            <p className={styles.hint}>
              <strong>Особенности:</strong> {pkg.traits.join(' · ')}
            </p>
            <p className={styles.hint}>
              <strong>Инструменты:</strong> {pkg.tools}
            </p>
            <p className={styles.hint}>
              <strong>Где встречается:</strong> {pkg.use}
            </p>
          </>
        )}
      </DemoCard>
...
function PkgGrid({pkgId, setPkgId}) {
  return (
    <div className={styles.modGrid}>
      {PACKAGE_MODELS.map((m) => (
        <button
          key={m.id}
          type="button"
          className={clsx(styles.modCard, pkgId === m.id && styles.modCardActive)}
          style={{'--mod-color': m.color}}
          onClick={() => setPkgId(m.id)}
        >
...
export default CppEcosystemPlayInner;
