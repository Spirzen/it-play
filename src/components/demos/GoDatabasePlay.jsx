...
function StackMode() {
  ...
  return (
    <>
      ...
      <div className={styles.stackDiagram} aria-label="Стек Go — database/sql — СУБД">
...
function CrudMode() {
  ...
  const [layerId, setLayerId] = useState('sql');
  ...
  const goCode = goCodeForLayer(layerId, opId, params);
      ...
      <div className={styles.crudForm}>
        {(opId === 'read' || opId === 'update' || opId === 'delete') && (
          <div className={styles.crudField}>
            <label htmlFor="go-db-id">id (пусто = все пользователи)</label>
            <input
              id="go-db-id"
              type="number"
              min="1"
        ...
        {(opId === 'create' || opId === 'update') && (
          <>
            <div className={styles.crudField}>
              <label htmlFor="go-db-name">name</label>
              <input
                id="go-db-name"
                value={form.name}
            ...
            <div className={styles.crudField}>
              <label htmlFor="go-db-email">email</label>
              <input
                id="go-db-email"
                value={form.email}
      ...
      <div className={styles.layout2}>
        ...
        <div>
          <span className={styles.panelLabel}>
            Go — {ACCESS_LAYERS.find((l) => l.id === layerId)?.label}
          </span>
          <pre className={styles.codePanel}>{goCode}</pre>
        </div>
      </div>
    </>
  );
...
function LayersMode() {
  const [opId, setOpId] = useState('read');
  const params = {id: 42, name: 'Dev', email: 'dev@go.dev'};
  ...
  return (
    <>
      ...
      <p className={styles.stepDetail} style={{textAlign: 'center', marginBottom: '0.65rem'}}>
        Одна операция — три подхода в экосистеме Go. Внизу всё равно database/sql и SQL.
      </p>
      ...
      <div className={styles.layerCompare}>
        {ACCESS_LAYERS.map((layer) => (
          <div key={layer.id} className={styles.layerCard}>
            ...
            <pre>{goCodeForLayer(layer.id, opId, params)}</pre>
          </div>
        ))}
      </div>

      <p className={styles.panelLabel} style={{marginTop: '0.75rem'}}>
        Итоговый SQL (пример для database/sql)
      </p>
...
function GoDatabasePlayInner() {
  const [mode, setMode] = useState('stack');

  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Go и база данных"
        subtitle="database/sql, пул соединений, CRUD и сравнение sql · GORM · sqlc"
      >
        ...
        <p className={styles.footer}>
          {mode === 'stack' &&
            'sql.DB — это пул, а не одно соединение. Драйвер регистрируется пустым импортом; Open не проверяет сеть до Ping или первого запроса.'}
          {mode === 'crud' &&
            'Плейсхолдеры $1, $2 защищают от SQL-инъекций. DSN и пароли — в окружении или vault, не в репозитории.'}
          {mode === 'layers' &&
            'GORM и sqlc ускоряют разработку; сгенерированный SQL всё равно нужно понимать и профилировать.'}
          {mode === 'lifecycle' &&
            'Закрывайте rows.Close() и не забывайте Rollback при ошибке в транзакции.'}
        </p>
      </DemoCard>
    </DemoShell>
  );
...
export default GoDatabasePlayInner;
