...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/swiftEcosystemEngine';
import styles from '@/components/demos/SwiftEcosystemPlay.module.css';
...
function SwiftEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('swiftui-ios');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('spm');
  ...
  const toggleWidget = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) next.add('widget');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема Swift-приложений"
        subtitle="Слои Apple-стека, граф модулей и SPM, структура iOS/Vapor/пакетов, сборка и публикация"
      >
        ...
        {tab === 'deps' && (
          <>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>WidgetExtension</code> — исчезнут связи
              виджета с SwiftUI и репозиторием.
            </p>
            <div className={styles.bundleBox}>
              ...
              <span className={styles.bundleChip}>SPM в графе: {bundle.spmCount}</span>
              <span className={styles.bundleChip}>SDK: {bundle.sdkCount}</span>
            </div>
          </>
        )}
...
export default SwiftEcosystemPlayInner;
