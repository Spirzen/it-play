...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/nimEcosystemEngine';
import styles from '@/components/demos/NimEcosystemPlay.module.css';
...
function NimEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('jester-api');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [pkgId, setPkgId] = useState('project');
  ...
  const pkg = PACKAGE_MODELS.find((p) => p.id === pkgId) ?? PACKAGE_MODELS[1];
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема и архитектура Nim"
        subtitle="Компиляция, модули, Nimble, фреймворки и сборка приложений"
      >
        ...
        {tab === 'deps' && (
          <>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>workers/</code> — исчезнут рёбра к redis и
              часть связей с services.
            </p>
...
export default NimEcosystemPlayInner;
