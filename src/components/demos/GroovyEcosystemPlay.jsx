...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/groovyEcosystemEngine';
import styles from '@/components/demos/GroovyEcosystemPlay.module.css';
...
function GroovyEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('gradle-groovy');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('gradle-dsl');
  ...
  const toggleSpock = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('spock');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема Groovy на JVM"
        subtitle="Слои стека, граф зависимостей, Gradle/Grails/Jenkins, сборка и способы подключения библиотек"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('spock')}
                  onChange={(e) => toggleSpock(e.target.checked)}
                />
                Тесты Spock (src/test/groovy)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в графе. Снимите Spock — исчезнут рёбра к spock-core и тестовому модулю.
            </p>
...
export default GroovyEcosystemPlayInner;
