...
import {
  ...
  runCrud,
  sqlForOp,
} from '@/components/shared/kb/javaDatabaseEngine';
...
function StackMode() {
  ...
  return (
    <>
      ...
      <div className={styles.stackDiagram} aria-label="Стек Java — JDBC — СУБД">
...
function CrudMode() {
  ...
  const javaCode = javaCodeForLayer(layerId, opId, params);
      ...
      <div className={styles.crudForm}>
        {(opId === 'read' || opId === 'update' || opId === 'delete') && (
          <div className={styles.crudField}>
            <label htmlFor="java-db-id">id книги (пусто = все available)</label>
            <input
              id="java-db-id"
              type="number"
              min="1"
        ...
        {(opId === 'create' || opId === 'update') && (
          <>
            <div className={styles.crudField}>
              <label htmlFor="java-db-title">title</label>
              <input
                id="java-db-title"
                value={form.title}
            ...
            <div className={styles.crudField}>
              <label htmlFor="java-db-author">author</label>
              <input
                id="java-db-author"
                value={form.author}
            ...
            {opId === 'create' && (
              <>
                <div className={styles.crudField}>
                  <label htmlFor="java-db-isbn">isbn</label>
                  <input
                    id="java-db-isbn"
                    value={form.isbn}
                ...
                <div className={styles.crudField}>
                  <label htmlFor="java-db-year">published_year</label>
                  <input
                    id="java-db-year"
            ...
            {opId === 'update' && (
              <div className={styles.crudField}>
                <label htmlFor="java-db-avail">available</label>
                <select
                  id="java-db-avail"
      ...
      <div className={styles.layout2}>
        ...
        <div>
          <span className={styles.panelLabel}>
            Java — {ACCESS_LAYERS.find((l) => l.id === layerId)?.label}
          </span>
          <pre className={styles.codePanel}>{javaCode}</pre>
        </div>
      </div>
    </>
  );
...
function LayersMode() {
  const [opId, setOpId] = useState('read');
  const params = {id: 2, title: 'Effective Java', author: 'Joshua Bloch', isbn: '978-0134685991', available: false};
      ...
      <div className={styles.layerCompare}>
        {ACCESS_LAYERS.map((layer) => (
          <div key={layer.id} className={styles.layerCard}>
            ...
            <pre>{javaCodeForLayer(layer.id, opId, params)}</pre>
          </div>
        ))}
      </div>
...
function JavaDatabasePlayInner() {
  const [mode, setMode] = useState('stack');

  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Java и база данных"
        subtitle="JDBC, пул соединений, CRUD и сравнение JDBC · JPA · Spring Data"
      >
        ...
        <p className={styles.footer}>
          {mode === 'stack' &&
            'Приложение не говорит с PostgreSQL напрямую — только через JDBC API и драйвер. Пул переиспользует TCP-соединения.'}
          {mode === 'crud' &&
            'PreparedStatement защищает от SQL-инъекций. В production пароли — только в конфиге или vault, не в коде.'}
          {mode === 'layers' &&
            'ORM и Spring Data сокращают boilerplate, но генерируемый SQL нужно понимать и профилировать.'}
          {mode === 'lifecycle' &&
            'Ресурсы закрывайте в try-with-resources: ResultSet → Statement → Connection.'}
        </p>
      </DemoCard>
    </DemoShell>
  );
...
export default JavaDatabasePlayInner;
