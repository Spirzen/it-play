...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/cEcosystemEngine';
import styles from '@/components/demos/CEcosystemPlay.module.css';
...
function CEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('daemon');
  ...
  const toggleCrypto = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('crypto');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Архитектура и экосистема С"
        subtitle="Слои стека, граф модулей, структура проекта, цепочка препроцессор → компилятор → линковщик"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('crypto')}
                  onChange={(e) => toggleCrypto(e.target.checked)}
                />
                Модуль crypto.c (OpenSSL)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в линковке. Отключите <code>crypto.c</code> — исчезнут зависимости от
              libssl и связи с db через шифрование.
            </p>
...
export default CEcosystemPlayInner;
