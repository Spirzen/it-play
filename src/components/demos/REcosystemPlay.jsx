...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/rEcosystemEngine';
import styles from '@/components/demos/REcosystemPlay.module.css';
...
function REcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('shiny');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('script');
  ...
  const togglePipeline = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('targets');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема аналитических приложений на R"
        subtitle="Слои стека, граф library/::, структура Shiny/пакета/targets/plumber, renv и конвейер сборки"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('targets')}
                  onChange={(e) => togglePipeline(e.target.checked)}
                />
                Конвейер targets (prepare_data → targets → arrow)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в приложении. Отключите <code>targets</code> — исчезнут рёбра к arrow;
              Shiny и dplyr останутся для UI.
            </p>
...
export default REcosystemPlayInner;
