...
import {
  ...
  runCrud,
  sqlForOp,
} from '@/components/shared/kb/rustDatabaseEngine';
...
function StackMode() {
  ...
  return (
    <>
      ...
      <div className={styles.stackDiagram} aria-label="Стек Rust — драйвер — СУБД">
...
function CrudMode() {
  ...
  const [layerId, setLayerId] = useState('tokio_postgres');
  ...
  const rustCode = rustCodeForLayer(layerId, opId, params);
      ...
      <div className={styles.crudForm}>
        {(opId === 'read' || opId === 'update' || opId === 'delete') && (
          <div className={styles.crudField}>
            <label htmlFor="rust-db-id">id книги (пусто = все available)</label>
            <input
              id="rust-db-id"
              type="number"
              min="1"
        ...
        {(opId === 'create' || opId === 'update') && (
          <>
            <div className={styles.crudField}>
              <label htmlFor="rust-db-title">title</label>
              <input
                id="rust-db-title"
                value={form.title}
            ...
            <div className={styles.crudField}>
              <label htmlFor="rust-db-author">author</label>
              <input
                id="rust-db-author"
                value={form.author}
            ...
            {opId === 'create' && (
              <>
                <div className={styles.crudField}>
                  <label htmlFor="rust-db-isbn">isbn</label>
                  <input
                    id="rust-db-isbn"
                    value={form.isbn}
                ...
                <div className={styles.crudField}>
                  <label htmlFor="rust-db-year">published_year</label>
                  <input
                    id="rust-db-year"
            ...
            {opId === 'update' && (
              <div className={styles.crudField}>
                <label htmlFor="rust-db-avail">available</label>
                <select
                  id="rust-db-avail"
      ...
      <div className={styles.layout2}>
        ...
        <div>
          <span className={styles.panelLabel}>
            Rust — {ACCESS_LAYERS.find((l) => l.id === layerId)?.label}
          </span>
          <pre className={styles.codePanel}>{rustCode}</pre>
        </div>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <p className={styles.stepDetail} style={{textAlign: 'center', marginBottom: '0.65rem'}}>
        Одна операция — три уровня: tokio-postgres, Diesel, SeaORM. Внизу всё равно SQL и драйвер.
      </p>
      ...
      <div className={styles.layerCompare}>
        {ACCESS_LAYERS.map((layer) => (
          <div key={layer.id} className={styles.layerCard}>
            ...
            <pre>{rustCodeForLayer(layer.id, opId, params)}</pre>
          </div>
        ))}
      </div>

      <p className={styles.panelLabel} style={{marginTop: '0.75rem'}}>
        Итоговый SQL (пример для PostgreSQL)
      </p>
...
function RustDatabasePlayInner() {
  const [mode, setMode] = useState('stack');

  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Rust и база данных"
        subtitle="tokio-postgres, пул bb8/r2d2, CRUD и сравнение Diesel · SeaORM"
      >
        ...
        <p className={styles.footer}>
          {mode === 'stack' &&
            'Rust не обращается к PostgreSQL напрямую — только через crate-драйвер и пул. tokio-postgres требует отдельной задачи для connection.await.'}
          {mode === 'crud' &&
            'Плейсхолдеры $1 и Diesel/SeaORM DSL защищают от SQL-инъекций. DATABASE_URL — из env или vault, не из git.'}
          {mode === 'layers' &&
            'Diesel проверяет схему при cargo build; SeaORM гибче в рантайме. Оба в итоге генерируют параметризованный SQL.'}
          {mode === 'lifecycle' &&
            'Ошибки БД — Result<_, DbErr>, не panic. Долгие транзакции держат блокировки: не включайте HTTP внутрь txn.'}
        </p>
      </DemoCard>
    </DemoShell>
  );
...
export default RustDatabasePlayInner;
