...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/dartEcosystemEngine';
import styles from '@/components/demos/DartEcosystemPlay.module.css';
...
function DartEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('flutter-mobile');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('pub');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Архитектура Dart-приложений"
        subtitle="Слои платформы, pub-зависимости, структура Flutter/CLI/shelf, JIT/AOT-сборка"
      >
        ...
        {tab === 'deps' && (
          <>
            ...
            <p className={styles.hint}>
              ...
              firebase_analytics и уменьшится размер APK.
            </p>
            <div className={styles.bundleBox}>
              ...
              <span className={styles.bundleChip}>pub-пакетов в графе: {bundle.pubCount}</span>
            </div>
          </>
        )}
...
export default DartEcosystemPlayInner;
