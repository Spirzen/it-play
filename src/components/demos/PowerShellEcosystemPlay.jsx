...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/powershellEcosystemEngine';
import styles from '@/components/demos/PowerShellEcosystemPlay.module.css';
...
function PowerShellEcosystemPlayInner() {
  ...
  const [pickedNode, setPickedNode] = useState('script');
  const [archId, setArchId] = useState('azure-deploy');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('scriptmodule');
  ...
  const mod = MODULE_SYSTEMS.find((m) => m.id === modId) ?? MODULE_SYSTEMS[1];
  ...
  const toggleTests = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('tests');
  ...
  const toggleCloud = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      ['az', 'graph', 'dsc'].forEach((id) => {
        if (on) next.add(id);
        else next.delete(id);
      });
      return next;
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема PowerShell"
        subtitle="Слои стека, граф модулей, структура Azure/M365/DSC, PSGallery, CI/CD и форматы модулей"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('tests')}
                  onChange={(e) => toggleTests(e.target.checked)}
                />
                Тесты Pester (Tests/ → Pester)
              </label>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('az') && enabledNodes.has('graph')}
                ...
                {DEP_NODES.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        setPickedNode(n.id);
                        if (n.id !== 'script') toggleNode(n.id);
                      }}
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в графе. Отключите <code>Tests/</code> — исчезнет связь с Pester; без Az/Graph
              останутся только локальные функции модуля.
            </p>
...
export default PowerShellEcosystemPlayInner;
