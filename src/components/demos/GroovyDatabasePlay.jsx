...
function StackMode() {
  const [scenarioId, setScenarioId] = useState('gorm_get');
  ...
  return (
    <>
      ...
      <div className={styles.stackDiagram} aria-label="Стек Groovy — GORM — JDBC — СУБД">
...
function CrudMode() {
  ...
  const [layerId, setLayerId] = useState('gorm');
  ...
  const groovyCode = groovyCodeForLayer(layerId, opId, params);
      ...
      <div className={styles.crudForm}>
        {(opId === 'read' || opId === 'update' || opId === 'delete') && (
          <div className={styles.crudField}>
            <label htmlFor="groovy-db-id">id книги (пусто = все available)</label>
            <input
              id="groovy-db-id"
              type="number"
              min="1"
        ...
        {(opId === 'create' || opId === 'update') && (
          <>
            <div className={styles.crudField}>
              <label htmlFor="groovy-db-title">title</label>
              <input
                id="groovy-db-title"
                value={form.title}
            ...
            <div className={styles.crudField}>
              <label htmlFor="groovy-db-author">author</label>
              <input
                id="groovy-db-author"
                value={form.author}
            ...
            {opId === 'create' && (
              <>
                <div className={styles.crudField}>
                  <label htmlFor="groovy-db-isbn">isbn</label>
                  <input
                    id="groovy-db-isbn"
                    value={form.isbn}
                ...
                <div className={styles.crudField}>
                  <label htmlFor="groovy-db-year">published_year</label>
                  <input
                    id="groovy-db-year"
            ...
            {opId === 'update' && (
              <div className={styles.crudField}>
                <label htmlFor="groovy-db-avail">available</label>
                <select
                  id="groovy-db-avail"
      ...
      <div className={styles.layout2}>
        ...
        <div>
          <span className={styles.panelLabel}>
            Groovy — {ACCESS_LAYERS.find((l) => l.id === layerId)?.label}
          </span>
          <pre className={styles.codePanel}>{groovyCode}</pre>
        </div>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <p className={styles.stepDetail} style={{textAlign: 'center', marginBottom: '0.65rem'}}>
        Одна операция — три уровня: GORM, groovy.sql.Sql и JDBC. Внизу — SQL.
      </p>
      ...
      <div className={styles.layerCompare}>
        {ACCESS_LAYERS.map((layer) => (
          <div key={layer.id} className={styles.layerCard}>
            ...
            <pre>{groovyCodeForLayer(layer.id, opId, params)}</pre>
          </div>
        ))}
      </div>
...
function GroovyDatabasePlayInner() {
  const [mode, setMode] = useState('stack');

  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Groovy и база данных"
        subtitle="GORM, Hibernate, groovy.sql.Sql, CRUD и поток запроса к PostgreSQL"
      >
        ...
        <p className={styles.footer}>
          {mode === 'stack' &&
            'GORM на JVM идёт через Hibernate и JDBC: доменный класс не ходит в PostgreSQL напрямую. Пул HikariCP переиспользует соединения.'}
          {mode === 'crud' &&
            'save() и groovy.sql.Sql с ? защищают от инъекций. Проверяйте hasErrors() и не полагайтесь на автосхему в продакшене.'}
          {mode === 'layers' &&
            'GORM сокращает CRUD, но динамические findBy* и ленивые hasMany порождают N+1 — смотрите SQL в логе.'}
          {mode === 'lifecycle' &&
            'withTransaction и @Transactional в сервисах — единая модель транзакций с Spring/Grails.'}
        </p>
      </DemoCard>
    </DemoShell>
  );
...
export default GroovyDatabasePlayInner;
