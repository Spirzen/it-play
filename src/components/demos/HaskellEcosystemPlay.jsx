...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/haskellEcosystemEngine';
import styles from '@/components/demos/HaskellEcosystemPlay.module.css';
...
function HaskellEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('servant');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('single');
  ...
  const mod = MODULE_SYSTEMS.find((p) => p.id === modId) ?? MODULE_SYSTEMS[0];
  ...
  const toggleEffects = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('effects');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема Haskell-приложений"
        subtitle="GHC и RTS, Hackage/Cabal/Stack, граф модулей, Servant/Yesod и конвейер компиляции Core → STG"
      >
          ...
          <>
            <div className={styles.toggleRow}>
              <label className={styles.checkLabel}>
                <input
                  type="checkbox"
                  checked={enabledNodes.has('effects')}
                  onChange={(e) => toggleEffects(e.target.checked)}
                />
                Слой эффектов (App.Effects · mtl · PostgreSQL)
              </label>
            </div>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>App.Effects</code> — исчезнут рёбра к mtl и postgresql-simple; домен остаётся чистым.
            </p>
...
export default HaskellEcosystemPlayInner;
