...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/jsEcosystemEngine';
import styles from '@/components/demos/JsEcosystemPlay.module.css';
...
function JsEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('react-vite');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('esm');
  ...
  const toggleReports = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) {
        next.add('reports');
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        title="Экосистема JavaScript"
        subtitle="Слои стека, граф модулей, структура SPA, сборка и форматы модулей"
      >
        ...
        {tab === 'deps' && (
          <>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>features/reports</code> — исчезнут рёбра к
              recharts и date-fns.
            </p>
            <div className={styles.bundleBox}>
              ...
              <span className={styles.bundleChip}>npm в бандле: {bundle.npmCount}</span>
            </div>
          </>
        )}
...
export default JsEcosystemPlayInner;
