...
import {
  ...
  defaultEnabledNodes,
  bundleSummary,
} from '@/components/shared/kb/kotlinEcosystemEngine';
import styles from '@/components/demos/KotlinEcosystemPlay.module.css';
...
function KotlinEcosystemPlayInner() {
  ...
  const [archId, setArchId] = useState('android-compose');
  const [selectedPath, setSelectedPath] = useState(null);
  const [buildStepId, setBuildStepId] = useState(BUILD_STEPS[0].id);
  const [modId, setModId] = useState('gradle');
  ...
  const toggleAnalytics = useCallback((on) => {
    setEnabledNodes((prev) => {
      const next = new Set(prev);
      if (on) next.add('analytics');
      else next.delete('analytics');
      return next;
    });
  }, []);
  ...
  return (
    <DemoShell className={styles.root}>
      <DemoCard
        ...
        subtitle="Слои стека, граф модулей и библиотек, структура Android/Ktor/KMM, сборка Gradle"
      >
        ...
        {tab === 'deps' && (
          <>
            ...
            <p className={styles.hint}>
              ...
              Клик по узлу переключает участие в сборке. Отключите <code>features/analytics</code> — исчезнут связи с
              ktor-client и serialization в этом feature.
            </p>
...
export default KotlinEcosystemPlayInner;
